...
# ============================================
# インデックス診断（全記事のGSC URL Inspection）
# 結果は docs/_indexing-history.json に蓄積。未掲載や状態変化があれば通知
# ============================================
log "インデックス診断中（全記事）..."
INDEX_SUMMARY=$(python3 "$SITE_DIR/scripts/check-indexing.py" 2>/dev/null || echo '{"error":"check-indexing.py failed"}')
INDEXING_REPORT=$(echo "$INDEX_SUMMARY" | python3 -c "
import json, sys
try:
    d = json.load(sys.stdin)
    total = d.get('total', 0)
    idx = d.get('indexed', 0)
    ni = d.get('not_indexed', 0)
    ni_slugs = d.get('not_indexed_slugs', [])
    changes = d.get('changes', [])
    lines = [f'インデックス済: {idx}/{total} (未掲載 {ni}件)']
    if ni_slugs:
        lines.append('未掲載: ' + ', '.join(ni_slugs[:5]) + ('...' if len(ni_slugs)>5 else ''))
    for c in changes:
        lines.append(f\"状態変化: {c.get('slug')}: {c.get('from')} -> {c.get('to')}\")
    print('\\n'.join(lines))
except Exception as e:
    print(f'インデックス診断パース失敗: {e}')
" 2>/dev/null)
log "$INDEXING_REPORT"

# 状態変化(改善 or 悪化)があったらDiscord通知
INDEX_CHANGES=$(echo "$INDEX_SUMMARY" | python3 -c "
import json, sys
try:
    d = json.load(sys.stdin)
    changes = d.get('changes', [])
    if changes:
        ...
        for c in changes:
            arrow = '→'
            lines.append(f\"・{c.get('slug')}: {c.get('from')} {arrow} {c.get('to')}\")
        print('\\n'.join(lines))
except: pass
" 2>/dev/null)
if [ -n "$INDEX_CHANGES" ]; then
  notify "インデックス状態変化検知\n${INDEX_CHANGES}" 3447003
fi
...
# ============================================
# ブログ素材コンテキスト取得（_blog-context.json）
# Mac側で `node scripts/build-blog-context.mjs` 実行 → git commit → サーバーで読み込む
# voices(approved) と furuta_voice.md を Claude プロンプトに必須素材として注入する
# ============================================
BLOG_CONTEXT_FILE="$SITE_DIR/scripts/_blog-context.json"
VOICES_BLOCK=""
FURUTA_TONE_BLOCK=""
if [ -f "$BLOG_CONTEXT_FILE" ]; then
  VOICES_BLOCK=$(BLOG_CONTEXT_FILE="$BLOG_CONTEXT_FILE" python3 -c "
import json, os
try:
    c = json.load(open(os.environ['BLOG_CONTEXT_FILE']))
    voices = c.get('voices', [])
    if not voices:
        print('（お客様の声データなし）')
    else:
        lines = []
        for v in voices:
            lines.append(f\"- [{v['id']}] {v['public_label']}（{v.get('industry','')}）\")
            if v.get('before'): lines.append(f\"  Before: {v['before']}\")
            if v.get('solution'): lines.append(f\"  施策: {v['solution']}\")
            if v.get('after'): lines.append(f\"  After: {v['after']}\")
            if v.get('quote'): lines.append(f\"  発言: 「{v['quote']}」\")
            if v.get('numbers'): lines.append(f\"  数値: {', '.join(v['numbers'])}\")
        print('\\n'.join(lines))
except Exception as e:
    print(f'（取得エラー: {e}）')
" 2>/dev/null)
  FURUTA_TONE_BLOCK=$(BLOG_CONTEXT_FILE="$BLOG_CONTEXT_FILE" python3 -c "
import json, os
try:
    c = json.load(open(os.environ['BLOG_CONTEXT_FILE']))
    print(c.get('furuta_tone', '') or '（口調サンプルなし）')
except: print('（取得エラー）')
" 2>/dev/null)
  log "ブログ素材: voices=$(echo "$VOICES_BLOCK" | grep -c '^- \[') / 古田口調=${#FURUTA_TONE_BLOCK}字"
else
  log "WARN: $BLOG_CONTEXT_FILE がない → 素材なしで生成（Mac側で build-blog-context.mjs 実行が必要）"
  VOICES_BLOCK="（お客様の声データなし）"
  FURUTA_TONE_BLOCK="（古田さん口調サンプルなし）"
fi
...
# ============================================
# 素材必須ポリシー（AI判定回避 — voices/古田口調を必ず本文に反映）
# ============================================
MATERIAL_POLICY='【素材必須ポリシー：AI生成判定を回避するための核心要件】

AI判定器（Originality.ai、GPTZero）および Googleの helpful content system は、テンプレ的・均質的なAI記事を低品質判定する。これを回避する最大の武器は「実在する素材を本文に組み込む」こと。本記事は以下を必ず満たすこと:

1. **お客様の声 最低1件を本文に組み込む**: 後段の「## 利用可能なお客様の声」リストから1〜2件選び、事例として本文に登場させる。public_label（業種＋役職等の匿名表記）を使い、本人の発言（quote）があれば引用形式で挿入する。架空事例の捏造は禁止——リストにあるものだけ使う。

2. **古田の口調・スタンスを本文に反映**: 後段の「## 古田の口調・スタンス・主張」を読み、その中の主張・口癖・嫌う表現を本記事に反映させる。特に「嫌う表現」セクションのフレーズは絶対に使わない。

3. **数値・固有名詞を最低3つ含める**: 一次情報の代替として、具体的な数字（時間・金額・回数・割合）と固有名詞（ツール名・業種・規模）を最低3つ盛り込む。

4. **冒頭の書き出しを毎回変える**: 「こんにちは、37Design代表の古田です。」「結論から言うと」のような固定の書き出しは禁止。記事ごとに異なる入り方をする（質問形・事例提示・データ提示・読者の悩み描写など）。

5. **見出し構造を多様化**: 同じH2構造の量産は典型的AI記事の特徴。H2の数も本文長も記事の主題に合わせて自然に変える。'
...
## 利用可能なお客様の声（この中から最低1件を本文に組み込む）
${VOICES_BLOCK}

## 古田の口調・スタンス・主張（必ず本文に反映する）
${FURUTA_TONE_BLOCK}
...
## 改善指示
狙うキーワード: ${KEYWORD}
改善ポイント: タイトル改善・meta改善・本文充実・内部リンク追加・**AI顧問への集約強化**・**AI判定回避（素材組み込み）**
リライト日: ${TODAY}
...
- 本文 **2500〜4500字**（主題に応じて自然に決める。長ければ良いというものではない）
- h2は **3〜6個**（記事タイプによって変える。how_toは多め、explainerは少なめ）。各h2にh3は0〜3個（不要な場合は付けない）
- 冒頭の書き出しは記事ごとに変える。「こんにちは古田です」のような固定の書き出しは禁止
...
- **必須素材**: 上記「利用可能なお客様の声」から最低1件を本文に登場させる（public_labelで匿名引用）。古田の口調・主張を本文に反映する
- **数値・固有名詞・日付を最低3つ**含める（時間/金額/割合/ツール名/業種/規模）
- 禁止表現: 絶対に/確実に/100%/必ず/業界No.1/結論から言うと/いかがでしたか/ぜひ参考にしてください/まとめると/重要なポイントは/〜と言えるでしょう/〜していきましょう/現代社会において/昨今の
...
## 記事の要件
- 本文 **2500〜4500字**（主題に応じて自然に決める。長ければ良いというものではない）
- h2は **3〜6個**（記事タイプに応じて。how_toは多め、explainerは少なめ）。各h2にh3は0〜3個（不要な場合は付けない）
- 構成: 導入 → 問題深掘り → 解決策 → **AI顧問への接続** → まとめ（ただし型通りに並べない。記事タイプ・主題に応じて自然に組み立てる）
- 冒頭の書き出しは記事ごとに変える。「こんにちは古田です」のような固定の書き出しは禁止
